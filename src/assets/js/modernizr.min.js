			...
			try {
				if ((c = !!a.canPlayType))
					(c = new Boolean(c)),
						...
						(c.m4a = (a.canPlayType('audio/x-m4a;') || a.canPlayType('audio/aac;')).replace(
							/^no$/,
							''
						))
			} catch (d) {}
	...
	return (
		...
		(function(a, b) {
			function l(a, b) {
				...
				return (
					(c.innerHTML = 'x<style>' + b + '</style>'), d.insertBefore(c.lastChild, d.firstChild)
				)
			}
			function m() {
				var a = s.elements
		...
		(e.testStyles = y),
		(g.className =
			g.className.replace(/(^|\s)no-js(\s|$)/, '$1$2') + (f ? ' js ' + v.join(' ') : '')),
		e
	)
})(this, this.document)),
	(function(a, b, c) {
