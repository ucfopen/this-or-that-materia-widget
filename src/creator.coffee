...
# Create an angular module to import the animation module and house our
# controller
...
# The 'Resource' service contains all app logic that does pertain to DOM
# manipulation
...
ThisOrThat.controller 'ThisOrThatCreatorCtrl',
	['$scope', '$timeout', '$sanitize', 'Resource',
		($scope, $timeout, $sanitize, Resource) ->
			...
			$scope.limitLength = ->
				$scope.questions[$scope.currIndex].title =
			...
			$scope.validation = (action, which) ->
				switch action
					...
					when 'change'
						if $scope.questions[which].title and
							$scope.questions[which].alt[0] and
							$scope.questions[which].alt[1] and
							$scope.questions[which].images[0] and
