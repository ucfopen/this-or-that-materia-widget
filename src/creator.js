...
ThisOrThat.factory('Resource', ['$sanitize',function($sanitize) {
	return {
		buildQset(title, items, isRandom) {
			...
			for (let item of items) {
				const processedItem = this.processQsetItem(item);
				if (processedItem) { qsetItems.push(processedItem); }
			}
...
ThisOrThat.controller('ThisOrThatCreatorCtrl', ['$scope','$timeout','$sanitize', 'Resource',function($scope, $timeout, $sanitize, Resource) {
	...
	materiaCallbacks.initNewWidget = (widget, baseUrl) => $scope.$apply(function() {
	...
	materiaCallbacks.onSaveClicked = function(mode) {
		...
		if (_isValid) {
			...
			const qset = Resource.buildQset($sanitize($scope.title), $scope.questions, $scope.randomizeOrder);
			if (qset) {
	...
	materiaCallbacks.onQuestionImportComplete = function(items) {
		for (let item of items) {
			...
			if (!item.questions || !item.answers) { return; }

			// gets the image URLs
			try {
				if (item.answers && item.answers[0] && item.answers[0].options && item.answers[0].options.asset) {
					_ids[0] = item.answers[0].options.asset.id;
					_urls[0] = Materia.CreatorCore.getMediaUrl(item.answers[0].options.asset.id);
				}
				if (item.answers && item.answers[1] && item.answers[1].options && item.answers[1].options.asset) {
			...
			$scope.questions.push({
				...
				images: _ids,
				isValid: true,
				alt: [item.answers[0].text, item.answers[1].text, (item.options != null ? item.options.feedback : undefined) || ''],
				URLs: _urls,
				id: item.id,
	...
	const _noTransition = () => (() => {
        ...
        for (let action in $scope.actions) {
            if (action !== 'activate') { result.push($scope.actions[action] = false); } else {
	...
	const _updateIndex = function(action, data) {
		switch (action) {
			case 'prev':
				if ($scope.currIndex > 0) { return $scope.currIndex--; } else { return $scope.currIndex = $scope.questions.length - 1; }
			case 'next':
				if ($scope.currIndex < ($scope.questions.length - 1)) { return $scope.currIndex++; } else { return $scope.currIndex = 0; }
			case 'select':
	...
	$scope.duplicate = function(index) {
		if ($scope.questions.length < 50) {
			...
			$timeout(() => _updateIndex('add', angular.copy($scope.questions[index]))
			, 200, true);
		}
	};
	...
	$scope.addQuestion = function(title, images, imgsFilled, isValid, alt, URLs, id, qid, ansid) {
		...
		if (imgsFilled == null) { imgsFilled = [false, false]; }
		if (isValid == null) { isValid = true; }
		if (alt == null) { alt = ["",""]; }
		if (URLs == null) { URLs = ["assets/img/placeholder.png","assets/img/placeholder.png"]; }
		if (id == null) { id = ""; }
		if (qid == null) { qid = ""; }
		if (ansid == null) { ansid = ""; }
		if ($scope.questions.length > 0) {
			if ($scope.questions.length < 50) {
				...
				$timeout(() => _updateIndex('add', { title, images, isValid, alt, URLs, id, qid, ansid })
				, 200, true);
			}
		} else {
			$scope.questions.push({
	...
	$scope.removeQuestion = function(index) {
		if (($scope.currIndex + 1) === $scope.questions.length) { $scope.actions.removelast = true; } else { $scope.actions.remove = true; }

		$timeout(_noTransition, 660, true);
		$scope.questions.splice(index, 1);

		if ($scope.currIndex === $scope.questions.length) {
			$timeout(() => _updateIndex('remove')
			, 200, true);
		}
	};
	...
	$scope.selectCurrent = function(index) {
		if (index > $scope.currIndex) { $scope.actions.slideright = true; }
		if (index < $scope.currIndex) { $scope.actions.slideleft = true; }

		$timeout(() => _updateIndex('select', index)
		, 200, true);
	...
	$scope.tutorialIncrement = function(step) {
		if ($scope.tutorial.step > 0) {
			switch (step) {
				case 1:
					if ($scope.tutorial.step === 1) { return $scope.tutorial.step++; }
					break;
				case 2:
					if ($scope.tutorial.step === 2) { return $scope.tutorial.step++; }
					break;
				case 3:
					if ($scope.tutorial.step === 3) { return $scope.tutorial.step++; }
					break;
				case 4:
					if ($scope.tutorial.step === 4) { return $scope.tutorial.step++; }
					break;
				case 5:
					if ($scope.tutorial.step === 5) { return $scope.tutorial.step++; }
					break;
				case 6:
					if ($scope.tutorial.step === 6) { return $scope.tutorial.step = null; }
					break;
			}
		} else { return false; }
	};

	$scope.limitLength = () => $scope.questions[$scope.currIndex].title = $scope.questions[$scope.currIndex].title.substring(0, 500);
	...
	$scope.validation = function(action, which) {
		switch (action) {
			case 'save':
				...
				if ($scope.dialog.invalid) { return false; } else { return true; }
			case 'change':
				if ($scope.questions[which].title && $scope.questions[which].alt[0] && $scope.questions[which].alt[1] && $scope.questions[which].images[0] && $scope.questions[which].images[1]) {
	...
	$scope.hideModal = () => $scope.dialog.invalid = ($scope.dialog.edit = ($scope.dialog.intro = false));
