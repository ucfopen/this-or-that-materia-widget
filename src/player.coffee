...
ThisOrThatEngine =
	angular.module 'ThisOrThatEngine', ['ngAnimate', 'hammer', 'ngSanitize']

ThisOrThatEngine.controller 'ThisOrThatEngineCtrl',
	['$scope', '$timeout', '$sanitize', ($scope, $timeout) ->
		$scope.gameState =
			ingame: false
			endgame: false
			score: 0
		...
		$scope.start = (instance, qset, version) ->
			...
			for item in _qset.items
				for answer in item.answers
					$scope.images.push(
						Materia.Engine.getImageAssetUrl answer.options.asset.id
					)
